...
import { BadRequestError, NotFoundError, UnauthorizedError, ValidationError, ConflictError, handleError } from "../utils/errors";
...
const SENSITIVE_SELECT = "-password -verificationToken -verificationTokenExpiry -resetToken -resetTokenExpiry -refreshToken";
...
export async function getUserStatus(req: AuthRequest, res: Response): Promise<void> {
  try {
...
export async function updateProfile(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    if (email && email !== user.email) {
      ...
      user.verificationTokenExpiry = new Date(Date.now() + VERIFICATION_TOKEN_EXPIRES_MIN * 60 * 1000);
      await user.save();
...
export async function deleteAccount(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const myMessages = await Message.find({ sender: myId }).select("attachments").lean();
    await deleteCloudinaryAttachments(myMessages.flatMap((m) => m.attachments ?? []));
    ...
    if (user.avatar) {
      const publicId = cloudinaryPublicIdFromUrl(user.avatar, "chat_app_profilePhoto");
      if (publicId) {
...
export async function updateAvatar(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const { password: _, verificationToken, verificationTokenExpiry, resetToken, resetTokenExpiry, refreshToken, ...userWithoutSensitive } = user.toObject();
...
export async function listBlockedUsers(req: AuthRequest, res: Response): Promise<void> {
  try {
