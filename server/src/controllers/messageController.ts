...
import { generateConversationPdf, createConversationPdfWriter } from "../services/export";
import cloudinary from "../config/cloudinary";
import { ForbiddenError, NotFoundError, ValidationError, handleError, sendError } from "../utils/errors";
...
export async function searchMessages(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const users = await User.find({ _id: { $in: messages.filter((m) => m.sender).map((m) => m.sender) } })
    ...
    const result = messages
      ...
      .map((m) => ({
        ...m,
        sender: m.sender ? userMap.get(m.sender.toString()) ?? null : null,
        room: {
          _id: m.room._id,
          name: m.room.name,
          type: m.room.type,
...
export async function searchRoomMessages(req: AuthRequest, res: Response): Promise<void> {
  try {
...
export async function exportRoom(req: AuthRequest, res: Response): Promise<void> {
  try {
    const { roomId } = req.params;
    ...
    const messageCount = await Message.countDocuments({ room: parsedRoomId.data });

    const safeTitle =
      roomTitle.replace(/[^\w\- ]+/g, "").replace(/\s+/g, "-").slice(0, 40) ||
      "conversa";
    ...
    for (;;) {
      ...
      lastId = (messages[messages.length - 1] as unknown as { _id: { toString(): string } })._id.toString();
    }
    end();
  } catch (error) {
    if (!res.headersSent) {
    ...
    logger.error({ error }, "Erro durante o streaming do PDF de exportação.");
...
export async function uploadAttachments(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const uploaded: { url: string; filename: string; mimetype: string; size: number; publicId: string }[] = [];
    ...
    if (error instanceof multer.MulterError) {
      sendError(res, 400, "VALIDATION_ERROR", "Arquivo muito grande (máximo 10MB).");
      return;
    }
...
export async function getThreadMessages(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    const room = await Room.findById(parsedRoomId.data).select("participants").lean();
    if (!room) {
      throw new NotFoundError("Sala não encontrada.");
    }
