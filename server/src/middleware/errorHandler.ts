...
import { AppError, ValidationError, toAppError, sendError } from "../utils/errors";
...
export function errorHandler(
  ...
  if (error instanceof AppError) {
    if (error.statusCode >= 500) {
      logger.error({ requestId, code: error.code, message: error.message, stack: error.stack }, "request error");
    }
  ...
  if (
    ...
    sendError(res, 400, "BAD_REQUEST", "JSON inválido no corpo da requisição.");
    return;
  }
  ...
  logger.error({ requestId, error, stack: error instanceof Error ? error.stack : undefined }, "erro não tratado");
