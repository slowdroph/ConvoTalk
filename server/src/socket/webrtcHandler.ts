...
function isParticipant(room: { participants: unknown[] } | null, userId: string): boolean {
...
const webrtcHandler = (io: SocketIOServer): void => {
  io.on("connection", (socket: Socket) => {
    ...
    socket.on(
      "call:initiate",
      async (
        data: { roomId: string; calleeId: string; callType: "audio" | "video" },
        ack?: (res: { error?: string; callId?: string }) => void,
      ) => {
        try {
          const parsed = safeParse(socketCallInitiateSchema, data);
          if (!parsed.success) {
            if (typeof ack === "function") ack({ error: parsed.error });
            return;
          }
          const { roomId, calleeId, callType } = parsed.data;

          if (calleeId === userId) {
            if (typeof ack === "function") ack({ error: "Chamada inválida." });
            return;
          }
          if (isInCall(userId)) {
    ...
    socket.on(
      ...
      async (
        ...
        try {
          ...
          if (!call || call.calleeId !== calleeId || calleeId !== userId) {
            if (typeof ack === "function")
    ...
    socket.on(
      ...
      async (
        ...
        try {
          ...
          const call = calls.get(callId);
          if (!call) return;
          if (call.calleeId !== calleeId || calleeId !== userId) return;
          removeCall(callId);
    ...
    const handleSignal = (
      ...
      socket.on(
        event,
        async (
          data: { callId: string; targetId: string; payload: unknown },
        ) => {
          try {
            const parsed = safeParse(socketWebRtcSignalSchema, data);
            if (!parsed.success) return;
            ...
            const expectedTarget = isCaller ? call.calleeId : call.callerId;
            if (targetId !== expectedTarget) return;

            io.to(call.roomId)
              .except(socket.id)
              .emit(event, { callId, roomId: call.roomId, from: userId, payload });
          } catch {
            // silencioso
          }
        },
