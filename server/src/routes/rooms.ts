...
router.delete("/:id/members/:userId", auth, validate(removeMemberParams), removeMember);
router.delete("/:id", auth, validate(deleteRoomParams), deleteRoom);
router.post("/:id/admins", auth, validate(addAdminSchema), addAdmin);
router.delete("/:id/admins/:userId", auth, validate(removeAdminParams), removeAdmin);
router.put("/:id/avatar", auth, upload.single("avatar"), updateGroupAvatar);
router.delete("/:id/avatar", auth, validate(deleteRoomParams), removeGroupAvatar);
