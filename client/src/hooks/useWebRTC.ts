...
export function useWebRTC({
  ...
  const getLocalStream = useCallback(async (type: CallType): Promise<MediaStream | null> => {
    if (!navigator.mediaDevices?.getUserMedia) {
      console.error("getUserMedia indisponível (contexto não seguro).");
      notifyRef.current?.(
        "error",
  ...
  const createPeer = useCallback(() => {
    ...
    peer.onicecandidate = (e) => {
      if (!e.candidate || !callIdRef.current || !peerIdRef.current) return;
  ...
  useEffect(() => {
    ...
    const handleIncoming = (data: IncomingCall & { roomId: string }) => {
      ...
      setIncomingCall({ callId: data.callId, callerId: data.callerId, callType: data.callType });
      setPhaseSafe("incoming");
    };

    const handleAccepted = (data: { callId: string; roomId: string; callType: CallType }) => {
      if (data.roomId !== roomId || data.callId !== callIdRef.current) return;
    ...
    const handleEnded = (data: { callId: string; roomId: string }) => {
      if (data.callId !== callIdRef.current && phaseRef.current !== "incoming") return;
      cleanup();
    };
    ...
    const handleOffer = async (data: {
      ...
      try {
        await setRemote(peer, data.payload as RTCSessionDescriptionInit);
