  ...
  return (
    <div className="fixed inset-x-0 top-0 z-50 h-dvh-fallback bg-zinc-950/95 flex flex-col">
      ...
      {phase === "active" && isVideo ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-6 px-6">
          ...
          <div className="text-center">
            ...
            {phase === "outgoing" && (
              <p className="text-zinc-400 text-sm mt-1">
                {isOtherOnline ? "Chamando..." : "Usuário indisponível no momento"}
              </p>
            )}
            {phase === "active" && (
              <p className="text-zinc-400 text-sm mt-1">Em chamada</p>
            )}
          </div>
          {phase === "active" && !isVideo && (
      ...
      <div className="flex items-center justify-center gap-4 px-4 pt-6 pb-[max(2.5rem,env(safe-area-inset-bottom))]">
        {phase === "incoming" ? (
          <>
            <ControlButton onClick={onReject} danger label="Recusar">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </ControlButton>
            <ControlButton onClick={onAccept} label="Aceitar" active>
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <>
            {phase === "active" && (
              <>
                <ControlButton
                  ...
                  active={muted}
                  label={muted ? "Ativar microfone" : "Silenciar"}
                >
                  {muted ? (
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16.5 15.5a4 4 0 000-7M19 11a4 4 0 010 3" />
                    </svg>
                  ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-14 0m7 7v4m-4 0h8M12 11a2 2 0 002-2V6a2 2 0 10-4 0v3a2 2 0 002 2z" />
                    </svg>
                  )}
                </ControlButton>
                {isVideo && (
            ...
            <ControlButton onClick={phase === "outgoing" ? onEnd : onEnd} danger label="Encerrar">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
