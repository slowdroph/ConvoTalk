      ...
      <div className="space-y-2">
        {lines.map((line, i) => (
          <div
            ...
            style={{ width: line.width, marginLeft: align === "right" ? "auto" : line.offset }}
          />
        ))}
      </div>
    </div>
...
function Avatar({ className, color = "#59e07b", ring = "#0e150e" }: { className?: string; color?: string; ring?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
    ...
    >
      ...
      <circle cx="12" cy="12" r="10" fill="url(#chatAvatarGrad)" stroke={ring} strokeWidth="2.5" />
...
export default function ChatIllustration() {
  return (
    <div className="relative w-full max-w-md mx-auto select-none" aria-hidden="true">
      <div className="relative rounded-3xl border border-zinc-700/60 bg-zinc-900/70 backdrop-blur-sm shadow-2xl overflow-hidden">
        <div className="flex items-center gap-1.5 px-4 py-3 border-b border-zinc-700/60">
          ...
          <div className="ml-3 flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-400 font-medium">Equipe</span>
        ...
        <div className="px-4 py-6 space-y-4">
          ...
          <div className="animate-float animation-delay-200 flex justify-end">
            <ChatBubble align="right" lines={[{ width: "60%", offset: 0 }]} />
          </div>

          <div className="animate-float animation-delay-400">
            <div className="flex items-end gap-2">
              <Avatar className="w-7 h-7 shrink-0" color="#7aa2f7" />
              <ChatBubble
                bg="#252c24"
                lines={[
                  { width: "85%", offset: 0 },
      ...
      <div className="absolute -top-4 -left-4 w-32 h-32 bg-green-600/10 rounded-full blur-3xl animate-glowPulse" style={{ animationDelay: "1.5s" }} />
    </div>
  );
}