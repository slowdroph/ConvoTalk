...
export default function Sidebar({ rooms, activeRoom, onSelectRoom, onConversationCreated, onGroupCreated, onDeleteRoom, unreadCounts, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-80 max-w-[85vw] bg-white border-r border-slate-200 flex flex-col h-full transition-transform duration-200 md:static md:translate-x-0 dark:bg-zinc-900 dark:border-zinc-700 [padding-top:env(safe-area-inset-top)] [padding-bottom:env(safe-area-inset-bottom)] ${
      ...
      >
        ...
        <RoomList rooms={rooms} activeRoom={activeRoom} onSelectRoom={onSelectRoom} onDeleteRoom={onDeleteRoom} unreadCounts={unreadCounts} />
        <CreateGroupModal
