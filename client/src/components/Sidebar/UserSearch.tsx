    ...
    <div className="px-2 py-2">
      <div className="flex items-center gap-2">
        <input
          ...
          maxLength={100}
          className="flex-1 px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white text-sm placeholder-zinc-500 focus:outline-none focus:border-green-500 transition-colors"
        />
        <button
          onClick={() => {
            setOpen(false);
      ...
      {query.length > 0 && !loading && results.length === 0 && (
        <p className="text-zinc-500 text-xs mt-2 px-1">Nenhum usuário encontrado.</p>
      )}
    </div>
  );
}
